...
simple_fn:
    add t0, x0, x0 # set t0 before using it
    mv a0, t0
    li a0, 1
    ret
...
# missing. Another hint: what does the "s" in "s0" stand for? --- saved
naive_pow:
    # BEGIN PROLOGUE
    addi sp, sp, -4
    sw s0, 0(sp)
...
inc_arr_loop:
    ...
    # Also ask yourself this: why don't we need to preserve t1? --- when we back to loop, slli will always update t1
    # We don't need to save t0 here, helper_fn uses t1
...
helper_fn:
    # BEGIN PROLOGUE
    addi sp, sp, -4
    sw s0, 0(sp) # s0 holds the base address, we need to save it
    # END PROLOGUE
    lw t1, 0(a0)
    addi s0, t1, 1
    sw s0, 0(a0)
